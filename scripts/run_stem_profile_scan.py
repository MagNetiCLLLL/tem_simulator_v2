...
def save_images(frame, output, parameters):
    ...
    for key, ax in zip(DETECTORS, axes):
        ...
        statistics[key] = dict(min_fraction=low, max_fraction=high,
            ...
            relative_std=float(values.std() / max(values.mean(), 1e-30)),
            michelson_modulation=float((high-low) / max(high+low, 1e-30)),
    ...
    fp = parameters.get("phonon_label") or (f"{parameters['phonons']} frozen phonons, seed {parameters['seed']}" if parameters["phonons"] else "Static ordered CIF (no frozen phonons)")
    ...
    fig, axes = plt.subplots(1, 3, figsize=(15, 5.5), constrained_layout=True)
    for key, ax in zip(DETECTORS, axes):
        image = ax.imshow(arrays[key + "_fraction"], cmap="gray", origin="lower",
                          interpolation="nearest", extent=extent, vmin=0,
                          vmax=statistics[key]["max_fraction"])
        ax.set_title(f"{key.upper()} · zero-based absolute scale\n"
                     f"Michelson modulation {statistics[key]['michelson_modulation']:.2%}")
        ax.set_xlabel("Sample X (nm)"); ax.set_ylabel("Sample Y (nm)")
        fig.colorbar(image, ax=ax, label="Fraction of emitted electrons", shrink=.8)
    fig.suptitle(f"{scope} · effective C1 {parameters['focus']['effective_defocus_mm'] * 1e6:.3f} nm\n"
                 "Zero is black; each panel's labelled maximum is white. No background subtraction.")
    fig.savefig(output / "haadf_df_bf_absolute_scale.png", dpi=180)
    plt.close(fig)
