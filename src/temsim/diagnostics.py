...
class PhysicalLayoutRecord:
    ...
    pole_vacuum_connector_outer_diameter_mm: float
    pole_vacuum_connector_axial_length_mm: float
...
def physical_layout_records(result) -> tuple[PhysicalLayoutRecord, ...]:
    ...
    for part in result.assembly.parts:
        ...
        records.append(PhysicalLayoutRecord(
            ...
            pole_vacuum_connector_outer_diameter_mm=max(float(part.data.get(
                "pole_vacuum_connector_outer_diameter_mm", 0.0,
            )), 0.0),
            pole_vacuum_connector_axial_length_mm=max(float(part.data.get(
                "pole_vacuum_connector_axial_length_mm", 0.0,
            )), 0.0),
