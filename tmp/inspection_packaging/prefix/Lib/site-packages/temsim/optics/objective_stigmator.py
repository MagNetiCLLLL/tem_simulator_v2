"""Canonical owned Objective Stigmator with a distributed quadrupole field."""
...
_DEFAULT_PART = module_manifest.part_data(
    _DEFAULT_OBJECTIVE_MODULE_PATH, "objective_stigmator"
)
_DEFAULT_SAMPLE = module_manifest.part_data(
    _DEFAULT_OBJECTIVE_MODULE_PATH, "sample"
)
_DEFAULT_CENTER_BELOW_SAMPLE_MM = (
...
class ObjectiveStigmatorDefinition:
    key: str = OBJECTIVE_STIGMATOR
    label: str = "Objective Stigmator"
    mechanical_center_below_sample_mm: float = _DEFAULT_CENTER_BELOW_SAMPLE_MM
    ...
    effective_length_mm: float = float(_DEFAULT_PART["effective_length_mm"])
    ...
    def create_component(self):
        return ObjectiveStigmatorComponent(
            name=self.label,
            key=self.key,
...
@dataclass
class ObjectiveStigmatorComponent(Stigmator):
    mechanical_center_below_sample_mm: float = _DEFAULT_CENTER_BELOW_SAMPLE_MM
    mechanical_length_mm: float = float(_DEFAULT_PART["length_mm"])
    mechanical_outer_diameter_mm: float = float(
        _DEFAULT_PART["mechanical_outer_diameter_mm"]
    ...
    optical_reference_z_mm: float = _DEFAULT_OPTICAL_REFERENCE_Z_MM
    corrector: str = "objective"

    EXPECTED_KEY: ClassVar[str] = OBJECTIVE_STIGMATOR
    ...
    def __setattr__(self, name, value):
        ...
        if ready and name == "mechanical_center_below_sample_mm":
            ...
            object.__setattr__(
                self,
                "optical_reference_z_mm",
                float(self.optical_reference_z_mm) + delta_mm,
            )
    ...
    def validate(self):
        ...
        if self.mechanical_center_below_sample_mm <= 0.0:
            raise ValueError(
                "Objective Stigmator must remain below the sample."
            )
        if self.mechanical_length_mm <= 0.0:
            raise ValueError(
                "Objective Stigmator mechanical length must be positive."
            )
        if not (
            0.0
            < self.mechanical_clear_bore_diameter_mm
            ...
            raise ValueError(
                "Objective Stigmator clear bore must fit inside its body."
            )
        if not 0.0 < self.length_mm <= self.mechanical_length_mm:
            raise ValueError(
                "Objective Stigmator field length must fit its body."
            )
        if self.max_strength_m2 <= 0.0:
            raise ValueError(
                "Objective Stigmator maximum strength must be positive."
            )
        return self
...
OBJECTIVE_STIGMATOR_DEFINITION = ObjectiveStigmatorDefinition()


def create_objective_stigmator():
    return OBJECTIVE_STIGMATOR_DEFINITION.create_component().validate()
...
def objective_stigmator_from_dict(data):
    ...
    component.key = OBJECTIVE_STIGMATOR
    component.name = "Objective Stigmator"
    component.corrector = OBJECTIVE_STIGMATOR_DEFINITION.owner
