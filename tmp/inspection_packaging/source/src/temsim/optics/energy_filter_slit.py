...
_DEFAULT_ENERGY_FILTER_SLIT_PART = module_manifest.part_data(
...
class EnergySelectionSlitComponent:
    ...
    distance_from_sector_exit_m: float = (
        float(_DEFAULT_ENERGY_FILTER_SLIT_PART["path_center_mm"]) * 1.0e-3
    )
    gap_m: float = 36.0e-6
    centre_m: float = 0.0
    zero_loss_offset_m: float = 0.0
    clear_height_m: float = (
        float(_DEFAULT_ENERGY_FILTER_SLIT_PART["clear_height_mm"])
        * 1.0e-3
    )
    maximum_gap_m: float = (
        float(_DEFAULT_ENERGY_FILTER_SLIT_PART["maximum_gap_mm"])
        * 1.0e-3
    )
    blade_thickness_m: float = (
        float(_DEFAULT_ENERGY_FILTER_SLIT_PART["blade_thickness_mm"])
        * 1.0e-3
    )
    ...
    def configure_energy_window(
        ...
        self.centre_m = (
            float(self.zero_loss_offset_m)
            + dispersion * centre * 1.0e-6
        )
...
def create_energy_selection_slit(
    ...
    distance_from_sector_exit_m=(
        float(_DEFAULT_ENERGY_FILTER_SLIT_PART["path_center_mm"])
        * 1.0e-3
    ),
):
...
def serialise_energy_selection_slit(slit):
    return {
        ...
        "zero_loss_offset_m": float(slit.zero_loss_offset_m),
